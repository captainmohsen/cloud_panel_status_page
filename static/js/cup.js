...
myApp.dashboard = (function($) {
  ...
  function init() {
    ...
    $_servertitle.append("<th style=\"width:9%\"> today</th>");
    for (var d = 6; d >= 1; d--) {
  ...
  */
  ...
  function buildServerHTML(data, ids) {
    ...
    while (stat = bar.pop()) {
      ...
      if (stat.len == 1) {
        ...
        if (stat.right - stat.left < 1000 * 3540) {
          ...
          stattip += " (" + new Number((stat.right - stat.left) / (1000 * 3600)).toFixed(1) + " hour)";
        }
    ...
    for (a = 0; a < uptimes.length; a++) {
      ...
      hours = minutes / 60;
      if (uptimes[a] >= 99.99) {
        uptimetext[a] = "available 100%";
      } else if (minutes < 60) {
        uptimetext[a] = "available " + new Number(uptimes[a]).toFixed(2) + "%<br>error " + new Number(minutes).toFixed(0) + " minute";
      } else {
        uptimetext[a] = "available " + new Number(uptimes[a]).toFixed(2) + "%<br>error " + new Number(hours).toFixed(1) + " hour";
      }
    }
  ...
  function Type2Word(t, icon) {
    ...
    case 2:
      return (icon ? "<span class=\"glyphicon glyphicon-ok-sign\"></span> " : "") + "ok";
      //case 99:
      //  return "unknown";
      //case 98:
      //  return "unknown";
